    ...
    egui::ScrollArea::both()
        ...
        .show(ui, |ui| {
            ...
            for key in 0..NUM_KEYS {
                ...
                let fill = if is_black { Color32::from_rgb(0x22, 0x22, 0x22) } else { Color32::from_rgb(0xcc, 0xcc, 0xcc) };
                painter.rect_filled(
                    Rect::from_min_size(Pos2::new(origin.x, y), egui::vec2(KEYS_WIDTH - 2.0, NOTE_HEIGHT - 1.0)),
                    0.0, fill,
                );
            }
            ...
            for beat in 0..=16u32 {
                ...
                let color = if beat % 4 == 0 { Color32::from_rgb(0x55, 0x55, 0x55) } else { Color32::from_rgb(0x33, 0x33, 0x33) };
                painter.line_segment(
                    [Pos2::new(x, grid_origin.y), Pos2::new(x, grid_origin.y + roll_height)],
            ...
            for key in 0..=NUM_KEYS {
                ...
                painter.line_segment(
                    [Pos2::new(grid_origin.x, y), Pos2::new(grid_origin.x + roll_width, y)],
            ...
            if let Some(pattern) = project.patterns.get(pattern_idx) {
                for note in &pattern.piano_roll.notes {
                    ...
                    painter.rect_filled(
                        Rect::from_min_size(Pos2::new(x, y + 1.0), egui::vec2(w - 1.0, NOTE_HEIGHT - 2.0)),
                        2.0, theme::FL_ORANGE,
                    );
                }
            }
                ...
                if let Some(pos) = response.interact_pointer_pos() {
                    ...
                    if rel_x >= 0.0 && rel_y >= 0.0 {
                        ...
                        if let Some(pattern) = project.patterns.get_mut(pattern_idx) {
                            pattern.piano_roll.notes.push(crate::project::pattern::Note {
