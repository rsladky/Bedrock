    ...
    ui.horizontal(|ui| {
        ...
        for i in 0..num_channels {
            ...
            ui.vertical(|ui| {
                ...
                ui.add_space(4.0);
                let (rect, _) =
                    ui.allocate_exact_size(egui::vec2(12.0, 60.0), egui::Sense::hover());
                ...
                let prev_vol = vol;
                ui.add(
                    egui::Slider::new(&mut vol, 0.0f32..=1.5)
                        .vertical()
                        .show_value(false),
                );
                if (vol - prev_vol).abs() > 1e-6 {
                    ch.volume = vol;
                    bridge.send(EngineCommand::SetMixerVolume {
                        channel_idx: i,
                        volume: vol,
                    });
                }
                ...
                // Mute
                let mute_color = if ch.muted {
                    Color32::from_rgb(0xff, 0x44, 0x00)
                } else {
                    theme::BG_LIGHT
                };
                if ui
